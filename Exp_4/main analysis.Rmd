...
```{r}
#Preregistered model
param_1_pid = EMC2::parameters(LBA_exp4_v1,selection="alpha",N = NULL)
...
array_theta_pid_1 <- array_theta_1 %>%
  ...
  add_column(datapoint = rep(1:15000,each=9,times=30))%>% #here 30'000 = number of chains (3) x number of iteration (10'000)
  rename(pid=subjects)
```
...
ggsave("Exp_4/figures/all_params_1.jpeg",
       width = 8, height = 6)
```
...
ggsave("Exp_4/figures/all_params_pid_1.jpeg",
       width = 8, height = 6)
```
...
```{r}
p3.1 <- ggplot(thresh_group_1, aes(x = exp(value), y = fct_rev(response))) +
...
## save a figure
 ggsave("Exp_4/figures/thresh.jpeg",width = 10, height = 6)
```
...
```{r}
p3.2 <- ggplot(thresh_diff_1, aes(x = exp(diff), y = "")) +  
...
p3.3 <- ggplot(thresh_diff_pid_1, aes(x = exp(diff), y = pid,
                                    fill=pid)) +  
...
# save a figure
 ggsave("Exp_4/figures/thresh_1.jpeg",
        width = 21, height = 10)
```
...
# save a figure
ggsave("Exp_4/figures/thresh_2.jpeg",
       width = 15, height = 10)
```
...
ggsave("Exp_4/figures/drift_rate_1.jpeg",
        width = 10, height = 6)
```
...
```{r}
p4.1 / p4.2
ggsave("Exp_4/figures/drift_rate_true_false_1.jpeg", 
       width = 16, height = 13)
```
...
# save a figure
ggsave("Exp_4/figures/drift_rat_mini_1.jpeg",
       width = 15, height = 10)
```
...
# save a figure
ggsave("Exp_4/figures/drift_rate_pid_1.jpeg",
       width = 15, height = 10)
```
