...
```{r}
raw <- list.files(here("Exp_3", "data"), pattern = "^raw.*csv", full.names = TRUE,
...
#Save raw data
```{r}
write_csv(raw,"Exp_3/data/raw_data_exp_3.csv")
```
...
#Save_data
```{r}
write_csv(raw, here("Exp_3", "data", "data_exp_3.csv"))
```
...
```{r}
data <- raw %>%
  mutate(fam = as.factor(as.character(fam)),
         cond = fct_recode(fam, Famous = "1", Novel = "0"),
         acc = case_when(
...
p.acc
ggsave("Exp_3/figures/acc_pid_barplot.jpeg",
       width = 8, height = 6)
```
...
p.acc_without <-ggplot(acc_pid_check_without, aes(x=pid, y=perc)) +
   ...
   ylab("Accuracy [%]") + ylim(c(0,100))
p.acc_without
ggsave("Exp_3/figures/acc_pid_barplot_without.jpeg",
       width = 8, height = 6)
```

#Saving raw data
```{r}
write_csv(data, "Exp_3/data/data_exp_3.csv")
data<-read_csv("Exp_3/data/data_exp_3.csv")
```
...
ggsave("Exp_3/figures/errors_distr_block.jpeg",
       width = 8, height = 8)
```
...
ggsave("Exp_3/figures/errors_distr.jpeg",
       width = 8, height = 8)
```
...
ggsave("Exp_3/figures/rt_line_pid.jpeg",
      width = 8, height = 8)
```
...
ggsave("Exp_3/figures/acc_line_pid_prob.jpeg",
       width = 8, height = 8)
```
...
ggsave("Exp_3/figures/rt_line_group.jpeg",
        width = 7, height = 8)
```
...
ggsave("Exp_3/figures/acc_line_group.jpeg",
        width = 7, height = 8)
```
...
ggsave("Exp_3/figures/rt_acc_line_group.jpeg",
        width = 15, height = 6)
```
...
ggsave("Exp_3/figures/rt_acc_line_dots.jpeg",
       width = 10, height = 6)
```
...
ggsave("Exp_3/figures/acc_barplot_group.jpeg",
        width = 6, height = 8)
```
...
ggsave("Exp_3/figures/rt_barplot_group.jpeg",
        width = 6, height = 8)
```
