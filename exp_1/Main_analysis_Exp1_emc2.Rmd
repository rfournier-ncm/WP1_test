...
```{r}
myColors <- c("Novel" = paletteer_d("rcartocolor::Safe",4)[1],"Learned" =  paletteer_d("rcartocolor::Safe",4)[2])
```
...
array_theta_pid_1 <- array_theta_1 %>%
  ...
  add_column(datapoint = rep(1:15000,each=9,times=30))%>% #here 15'000 = number of chains (3) x number of iteration (5'000)
  rename(pid=subjects)
```
...
## plot the average difference and pid differences per condition##


```{r}
drift_acc_cond_diff_pid_1$pid = with(drift_acc_cond_diff_pid_1, reorder(pid, Novel, mean))
p4.8 <- ggplot(drift_acc_cond_diff_1, aes(x = Novel, y=pid)) +  
  stat_halfeye(alpha = 0.9, fill="firebrick") +
  stat_halfeye(data = drift_acc_cond_diff_pid_1,
               alpha = 0.7, fill = paletteer_d("rcartocolor::Safe",4)[1]) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Individual and average 
drift rate for Novel faces", 
       x = "Drift rate", y="participant ID") 
p4.8

ggsave("Exp_1/figures/drift_rat_ind_novel.jpeg",
       width = 15, height = 10)

drift_acc_cond_diff_pid_1$pid = with(drift_acc_cond_diff_pid_1, reorder(pid, Learned, mean))
p4.9 <- ggplot(drift_acc_cond_diff_1, aes(x = Learned, y=pid)) +  
  stat_halfeye(alpha = 0.9, fill="firebrick") +
  stat_halfeye(data = drift_acc_cond_diff_pid_1,
               alpha = 0.7, fill = paletteer_d("rcartocolor::Safe",4)[2]) +
  geom_vline(xintercept = 0, color = "darkgrey") +
  labs(title = "Individual and average 
drift rate for Learned faces", 
       x = "Drift rate", y="participant ID") 
p4.9
ggsave("Exp_1/figures/drift_rat_ind_learned.jpeg",
       width = 15, height = 10)
...
## Multivel Bayesian estimation approach
