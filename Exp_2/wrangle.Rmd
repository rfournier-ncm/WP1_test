...
```{r}
raw <- list.files(here("Exp_2", "data", "data_exp_2", "data"), pattern = "*csv", full.names = TRUE,
...
#Saving raw data
```{r}
write_csv(raw, "Exp_2/data/data_exp_2/raw_data_exp_2.csv")
```
...
wrangle <- raw %>%
  ...
  rename(keypress = response_recog.keys, acc = response_recog.corr, rt = response_recog.rt, pid = `participant`) %>%
  relocate(pid, .before = keypress) %>%
  select(pid, keypress, acc, rt) %>%
  mutate(block = rep(1:3, each = 60, times = 35),
         keypress = as.factor(keypress),
         cond = if_else(keypress == 'q' & acc == 1, "Learned",
                        if_else(keypress == 'p' & acc == 0, "Learned",
                        if_else(keypress == 'p' & acc == 1, "Novel",
                        if_else(keypress == 'q' & acc == 0, "Novel","")))))%>%
...
#Saving raw data
```{r}
write_csv(wrangle, "Exp_2/data/data_exp_2/wrangle_data_exp_2.csv")
```
...
#Check if all response times > 2 and < .02 have been correctly not recorded
```{r}
exp_2_data <- wrangle %>% 
...
removing participants with less than 75% of RT < 2s
```{r}
rt_check <- exp_2_data %>%
  group_by(pid) %>%
  summarise(sum_rt = sum(is.na(rt_OSF)),
            perc = sum_rt*100/180)
rt_check
#Participants: 5ba33d995196c600014853b6 / 61646e7ec3accfb9d8334ad5 / 66bb63dab2b110041a61af00 / 685b607b592c100ec12e663f have more than 25% of RT < 2s

exp_2_data <- exp_2_data %>%
  filter(!pid == "5ba33d995196c600014853b6",
         !pid == "61646e7ec3accfb9d8334ad5",
         !pid == "66bb63dab2b110041a61af00",
         !pid == "685b607b592c100ec12e663f")
```

removing participants with less than 55% accuracy
```{r}
#Remove NA from dataframe and check accuracy
exp_2_data <- exp_2_data %>%
  filter(!is.na(rt_OSF)) %>%
  select(!rt_OSF)

accuracy_check <-exp_2_data %>%
  group_by(pid) %>% 
  summarise(sum = sum(acc),
            n = n(), # n here is the total trials per grouping
            perc = sum/n*100)
#Participant 678862b1eccd9d65d8fcfa83 has less than 55% accuracy

exp_2_data <- exp_2_data %>%
  filter(!pid == "678862b1eccd9d65d8fcfa83")
```

##Saving data
```{r}
#Saving data
write_csv(exp_2_data, "Exp_2/data/data_exp_2.csv")
#Loading data
data<-read_csv("Exp_2/data/data_exp_2.csv")
```
## Accuracy 
...
data <- data %>% 
  mutate(acc = case_when(
         ...
         TRUE ~ 0))%>%
  mutate(cond = fct_relevel(cond,"Learned", "Novel")) %>%
...
#Save data model
```{r}
write_csv(data, "Exp_2/data/data_model.csv")
```
...
ggsave("Exp_2/figures/errors_distr_block.jpeg",
       width = 8, height = 8)
```
...
ggsave("Exp_2/figures/errors_distr.jpeg",
       width = 8, height = 8)
```
