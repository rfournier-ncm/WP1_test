...
#Bias model
param_2_pid = EMC2::parameters(LBA_exp1_v1_bias, selection = "alpha", map = TRUE)
```
...
array_theta_pid_2 <- array_theta_2 %>%
  pivot_longer(cols = c("v_Sunf_lMTRUE", "v_Sunf_lMFALSE", "v_Sfam_lMTRUE", "v_Sfam_lMFALSE", "sv_lMTRUE", "B_lRunf", "B_lRfam", "A_lRunf", "A_lRfam", "t0"), names_to = "param", values_to = "value")%>%
  add_column(datapoint = rep(1:30501,each=10,times=30))%>%
  rename(pid=subjects)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/all_params_2.jpeg",
       width = 8, height = 6)
```
...
ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/all_params_pid_2_1.jpeg",
       width = 8, height = 6)
```
...
thresh_pid_2 <- array_theta_pid_2 %>% 
  ...
  mutate(response = if_else(str_detect(param, "lRunf"), "unf_key", "fam_key"),
         response = factor(response,
                            levels = c("unf_key", "fam_key")))
head(thresh_pid_2)
str(thresh_pid_2)
...
thresh_group_2 <- theta_group_2 %>% 
  filter(str_detect(param, "B_")) %>% 
  mutate(response = if_else(str_detect(param, "lRfam"), "fam_key", "unf_key"),
         response = factor(response,
                            levels = c("fam_key", "unf_key")))
head(thresh_group_2)
str(thresh_group_2)
...
## save a figure
 ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/thresh2.jpeg",width = 10, height = 6)
```
...
# save a figure
 ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/thresh2.jpeg",
        width = 21, height = 10)
```
...
# save a figure
ggsave("/Users/rfournier/Rstudio/EMC_WP1_1/EMC_WP1_1/figures/thresh2_2.jpeg",
       width = 15, height = 10)
```
...
drift_rate_pid_2 <- array_theta_pid_2 %>% 
  ...
  mutate(condition = if_else(str_detect(param, "unf"), "unf", "fam"),
...
ggsave("figures/drift_rate_2.jpeg",
        width = 10, height = 6)
```
...
```{r}
p4.1 / p4.2
ggsave("figures/drift_rate_true_false_2.jpeg", 
       width = 16, height = 13)
```
...
# save a figure
ggsave("figures/drift_rat_mini_2.jpeg",
       width = 15, height = 10)
```
...
# save a figure
ggsave("figures/drift_rate_pid_2.jpeg",
       width = 15, height = 10)
```
...
## save a figure
 ggsave("/Users/rfournier/Rstudio/pilot5_dmc/figures/bias_2.jpeg",width = 10, height = 6)
```
...
## Compute decision threshold (Threshold parameter + Starting point)
```{r}
