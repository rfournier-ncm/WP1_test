...
pkgs <- c(
  "tidyverse", "RColorBrewer", "patchwork", "gtsummary", "paletteer", "eeptools", "here", "gridExtra", "EMC2", "tidybayes", "styler", "tinytable", "osfr"
)
vapply(
...
total_part_exp1    <- exp1_row$`Final n`
...
total_number_part             <- exp2_row$Started
inform_consent                <- exp2_row$Withdrew
number_part_compl             <- exp2_row$Completed
part_trial_25                 <- exp2_row$`RT exclusions`
part_acc_55                   <- exp2_row$`Accuracy exclusions`
number_participant_analysis_2 <- exp2_row$`Final n`
...
participant_n_exp3   <- exp3_row$Started
number_participant_3 <- exp3_row$`Final n`
...
total_number_part_4           <- exp4_row$Started
inform_consent_4              <- exp4_row$Withdrew
number_part_compl_4           <- exp4_row$Completed
part_trial_25_4               <- exp4_row$`RT exclusions`
part_acc_55_4                 <- exp4_row$`Accuracy exclusions`
ratings_only_4                <- as.integer(exp4_row$`Familiarity exclusions`)
number_participant_analysis_4 <- exp4_row$`Final n`
...
#| tbl-cap: "Participant summary across experiments. RT = response time; — = criterion not applicable for this experiment."
#| include: true
...
For all experiments, posterior distributions for drift rate and threshold parameters are visulaised in @fig-threshold-all and @fig-drift-all and the medians and 95% quantile intervals are summarised in @tbl-lba-params.
...
download_if_missing("exp_1", "LBA_exp1_v1.RData", "exp_1/models")
download_if_missing("exp_2", "LBA_exp2_v1.RData", "exp_2/models")
download_if_missing("exp_4", "LBA_exp4_v1.RData", "exp_4/models")
```
...
theta_all <- bind_rows(
  wrangle_lba(LBA_exp1_v1, 1, "Learned"),
  wrangle_lba(LBA_exp2_v1, 2, "Learned"),
  wrangle_lba(LBA_exp4_v1, 4, "Famous")
)
```
...
#| tbl-cap: "Posterior median and 95% credible intervals for threshold and drift rate parameters across experiments. Positive contrast values indicate a novel face advantage."
#| include: true
...
thresh_table <- thresh_diff_all |>
  ...
  mutate(
    ...
    condition = if_else(experiment == 4, "novel - famous", "novel - learned")
  ) |>
...
*Drift rate:* Inspection of the posterior distribution shows that there is a familiarity effect in drift rate difference, which mirrors the results from Experiment 1 and Experiment 2. The quality of the evidence accumulation was greater for Novel faces compared to Learned faces.
